...
static int __apb_prescaler(int prescaler) {
	...
	return map_reg_val(map, sizeof(map)/sizeof(map[0]),
			   prescaler);
}
...
int stm32f10x_clock_control_init(struct device *dev)
{
	...
	rcc->cfgr.bit.pllmul =
		__pllmul(CONFIG_CLOCK_STM32F10X_PLL_MULTIPLIER);
	...
	/* setup AHB prescaler */
	rcc->cfgr.bit.hpre =
		__ahb_prescaler(CONFIG_CLOCK_STM32F10X_AHB_PRESCALER);

	/* setup APB1, must not exceed 36MHz, prescaler set to 0 */
	rcc->cfgr.bit.ppre1 =
		__apb_prescaler(CONFIG_CLOCK_STM32F10X_APB1_PRESCALER);

	/* setup APB2 to use 36MHz, prescaler set to 0 */
	rcc->cfgr.bit.ppre2 =
		__apb_prescaler(CONFIG_CLOCK_STM32F10X_APB2_PRESCALER);
...
/**
 * @brief obtain the rate of the clock driving subsystem @subsys
 */
...
static struct stm32f10x_rcc_data stm32f10x_rcc_data_0 = {
 ...
 */
DEVICE_INIT(rcc_stm32f10x_0, "RCC", &stm32f10x_clock_control_init,
            &stm32f10x_rcc_data_0, NULL,
            PRIMARY, 1);
