 ...
 */
static inline void __setup_pins(const struct stm32_board_pinconf *pinconf,
				size_t pins)
{
	struct device *clk;
	int i;
	...
	for (i = 0; i < pins; i++) {
		__pinmux_stm32_set(pinconf[i].pin, pinconf[i].afunc, clk);
	}
}
...
{
	size_t pins = 0;
	const struct stm32_board_pinconf *pinconf;
