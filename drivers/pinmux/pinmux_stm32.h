...
/**
 * @brief pin function assignment wrapper for the target board
 */
struct stm32_board_pinconf {
	uint32_t pin;		/* pin ID STM32PIN() encoded */
	uint32_t afunc;		/* alternate function */
};

/**
 * @brief helper for obtaining pin configuration for the board
 *
 * @param pins out  set to the number of pins in the array
 *
 ...
 * in the configuration arrray.
 *
 * @return array of pin assignments
 */
const struct stm32_board_pinconf *stm32_board_get_pinconf(size_t *pins);
