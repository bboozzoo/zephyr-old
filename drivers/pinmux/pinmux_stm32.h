 ...
 *
 * GPIO function is assumed to be always avaialble, as such it's not listed
 * in `funcs` array
 */
struct stm32_pinmux_conf {
	uint32_t pin;		 /* pin ID */
	stm32_pin_func_t *funcs; /* functions array, indexed with
				  * (stm32_pin_alt_func - 1)
				  */
	size_t nfuncs;		 /* number of alternate functions, not
				  * counting GPIO
				  */
};
 ...
 *
 * Helper function for mapping alternative function for given pin to
 * its configuration. This function must be implemented by your soc*
 * code.
 *
...
/**
 * @brief helper for mapping port to clock subsystem
 *
 * @param port  IO port
 *
